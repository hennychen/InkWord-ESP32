...
#define LR_MAGIC   0x4C523034u  /* "LR04" */
 ...
 * lr_state blob 最坏 8B 头 + 21B×4000 = 84,008B，双写余量充足；
        ...
        if (buf) {
            if (nvs_get_blob(h, key, buf, &len) == ESP_OK) {
                ...
                if (magic == LR_MAGIC &&
                    ...
                    len >= LR_HDR + (size_t)used * sizeof(lr_sparse_t)) {
                    unpack(buf, used);
                    LOG_I("restored learning state: %d words (%d active, deck=%s)",
