 ...
 * 进入，激活期间独占七键。栈串联制（2026-09-08）：启动子功能时菜单
 * 保持入栈、子功能页入栈其上（page_router_push），退出=pop 一层
 * render_top 自动返回菜单（原「先 exit 后 enter」互斥 overlay 时代
 * 纪律退役；终结型动作——模式/词书确认、墨封当前词——仍自退回 base）。
