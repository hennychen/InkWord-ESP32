 ...
 *   出处 [200,192,192,24)：右下角右对齐 "——王阳明《传习录》"
 ...
 * 局刷路径真机实测显示异常且有残影，弃用）：
 *   - 引文下标 = epoch / STANDBY_QUOTE_INTERVAL_S(300s) % 24（无状态，
 *     重启/校时自然对齐同一窗口）；变化即整页全局刷新（GxEPD2
 *     display(false) 全刷模式写 previous，自带残影清理），一天 288 次
