 ...
 * （默认 gfx 320x240 横；用户改方向后 240x320 竖）与 4.2"/3.7"（横）
 * 同落 MID 档为首个共存案例。
 * 消费方统一读 form 字段，禁止再散落手写比较（T1.5 收敛
 * 三元宏同理）。同档宽度差也是真约束：MID 档现役最宽 416、最窄 320，
 * 2026-09-19 已把两处 416 基线写死处收敛为宽度判据（kb_scale 钳制、
 * LAN 页 ASCII ui_fit_font 降级 + 折行）；竖屏 MID 字号/几何参数
 * 仍待 3.1" 真机校准，若与横屏基线差异大再拆竖屏专属条目（k_profiles
 * 表按 kind 一维，form 是运行期覆盖字段而非独立档位，避免 4×3 稀疏
 * 矩阵） */
