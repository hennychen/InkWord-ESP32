...
{
    /* 字库六级上限 48px（cjk_font.h）：宽字形 ts*cell = 6*48 = 288B。
     * 旧容量 32*4=128B 是四级化时代残留——40/48px 级宽字形 memset 写爆
     * 栈缓冲，冲掉相邻局部变量（坐标/颜色），表现为词卡释义字形裁切
     * （窄字形如音标不溢出，故左栏正常；2026-09-17 真机实证） */
    uint8_t tmp[6 * 48];
    int ts = (w + 7) / 8;
