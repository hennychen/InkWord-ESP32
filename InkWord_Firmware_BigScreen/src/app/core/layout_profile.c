...
#include "epd_gfx.h"   /* 大屏迁移：小屏 epd_driver.h GFX 段同签名实现 */
...
static const layout_profile_t k_profiles[] = {
    ...
    [LAYOUT_XLARGE] = { LAYOUT_XLARGE, LAYOUT_FORM_LANDSCAPE, 3, 4, 3,
                        120, 80, 140, 160, 48, 48, 3, 32, 4, 48, 0, 0, 100,
                        100, 150, 125, 400 },
                     /* 10.1" 1920x1080 大屏（2026-11 迁移新增）：reader_level
                      * 2026-09-17 UI 重设计 3→4（六级字库 40px 落地，物理
                      * 字高 40px≈4.6mm @218PPI 大屏阅读主体）；几何按
                      * 大屏专属词卡布局取值（状态栏 120/留白 80/列表行
                      * 160/ascii 徽标 24pt）。PPI 自动层未注入 dpi（大屏
                      * 未标定）→ 表值直效 */
};
...
const layout_profile_t *layout_profile_get(void)
{
    if (!s_ready) {
        ...
        if (lvl >= 0) {
            ...
            static const int item_pad[] = {12, 16, 24, 20, 48};
