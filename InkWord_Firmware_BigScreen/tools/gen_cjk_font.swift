// gen_cjk_font.swift —— 大屏版中文点阵字库生成器（六级 16/20/24/32/40/48px；
// 40/48px 级 2026-09-17 增：UI 重设计——词卡释义 48px / 阅读正文 40px /
// 待机引文 48px，楷体大字链。自小屏 InkWord_Firmware/tools 同名脚本复制
// 改造：LEVELS 扩级 + 词库/bin 路径适配大屏目录 + 裁剪 c/h/引文表生成段
// （大屏 cjk_font.c/h 与 src/app/core/quotes_app.* 为手维护版本，
// 生成器只产 bin；上游脚本改进时手工同步本文件）
//
// 输入：tools/chuanxilu_quotes.txt（引文字符集收录用，与大屏引文表同源）
// 输出：src/app/data/cjk_font_data.bin（二进制字库：头 + 码点表 + 六级位图）
//
...
let LEVELS = [16, 20, 24, 32, 40, 48]   // 像素格边长（level 0~5；40/48px 级
                      // 大屏 UI 重设计 2026-09-17：释义/引文 48px、阅读正文 40px）
let FONT_SIZE_HINT: [Int: CGFloat] = [16: 15, 20: 19, 24: 22, 32: 30, 40: 38, 48: 45]
...
if let jsonData = try? Data(contentsOf: URL(fileURLWithPath: "src/app/data/default_words.json")),
...
let fontsDesc = "16/20px \(fontLabel(16)) + 24/32/40/48px \(fontLabel(24))"
...
try! bin.write(to: URL(fileURLWithPath: "src/app/data/cjk_font_data.bin"))

/* 大屏版裁剪（2026-09-17）：不生成 cjk_font.c/h 与 quotes_app.* ——
 * 大屏 src/app/core/cjk_font.c/h 与 quotes_app.* 为手维护版本，
 * 本生成器只产 bin（含 CJK_FONT_LEVELS 等宏的同步见下方报告） */
