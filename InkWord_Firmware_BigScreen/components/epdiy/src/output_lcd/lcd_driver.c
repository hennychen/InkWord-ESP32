...
// [InkWord 编译适配] IDF 5.5 兼容（仅编译层，驱动逻辑保持卖家原样）：
//   IDF 5.3.2 起 lcd_periph_signals 改名 lcd_periph_rgb_signals；
//   外设时钟改用 PERIPH_*_MODULE 枚举；gpio_hal_iomux_func_sel 改用 gpio_ll_func_sel；
//   lcd_ll_set_data_width 改用 lcd_ll_set_data_wire_width（5.3+）；
//   lcd_hal_cal_pclk_freq 第 4 参改为 hal_utils_clk_div_t 指针且需回写分频系数（5.3+）。
...
// [run71 诊断] ISR 活动计数器：死锁时区分「LCD 引擎未跑」vs「EOF 停」vs「vsync 停」
volatile uint32_t g_lcd_dbg_vsync = 0;
volatile uint32_t g_lcd_dbg_eof = 0;
// [run79 诊断] vertical timing 回读：start_frame 写入后 vs 边界写入后
// 的 lcd_va_height 寄存器值（k>=1 初批 vact 被忽略疑云）
volatile uint32_t g_dbg_va_start = 0;
volatile uint32_t g_dbg_va_start_rb = 0;
volatile uint32_t g_dbg_va_after_reset = 0;
volatile uint32_t g_dbg_va_after_start = 0;
volatile uint32_t g_dbg_va_bnd_rb = 0;

void epd_lcd_dbg_va(uint32_t* wr, uint32_t* rb_start, uint32_t* rb_bnd) {
    if (wr) *wr = g_dbg_va_start;
    if (rb_start) *rb_start = g_dbg_va_start_rb;
    if (rb_bnd) *rb_bnd = g_dbg_va_bnd_rb;
}

void epd_lcd_dbg_va2(uint32_t* after_reset, uint32_t* after_start) {
    if (after_reset) *after_reset = g_dbg_va_after_reset;
    if (after_start) *after_start = g_dbg_va_after_start;
}

void epd_lcd_dbg_counters(uint32_t* vsync, uint32_t* eof, uint32_t* batches) {
    if (vsync) *vsync = g_lcd_dbg_vsync;
    if (eof) *eof = g_lcd_dbg_eof;
    if (batches) *batches = (uint32_t)lcd.batches;
}

void epd_lcd_dbg_pins(int* ckv, int* stv) {
    if (ckv) *ckv = gpio_get_level(lcd.config.bus.ckv);
    if (stv) *stv = gpio_get_level(lcd.config.bus.stv);
}

/// The number of bytes in a horizontal display register line.
static int line_bytes = 0;
static int vertical_lines = 0;
// [run79] 对齐上游 main：每行 dummy 字节数 = bus_width/8（16bit 总线=2）。
// 卖家 epdiy2 旧逻辑 (bus_width==8) 在 IDF 5.3+ 上行走不通。
static int dummy_bytes = 0;
// [run79] 上游 main 帧启动自旋锁：保证 CKV/STV/引擎启动相位确定
...
// [run78] 边界重武装任务：已废弃（run79 改上游 main 初始化序列后
// 边界回归 ISR 直启，与上游一致）。
...
void IRAM_ATTR epd_lcd_start_frame() {
    ...
    // [run79] 对齐上游 main：初批 vbp=0（卖家旧值 1）
    lcd_ll_set_vertical_timing(lcd.hal.dev, 1, 0, initial_lines, 1);
    g_dbg_va_start = initial_lines;
    g_dbg_va_start_rb = lcd.hal.dev->lcd_ctrl.lcd_va_height;
    ...
    g_dbg_va_after_reset = lcd.hal.dev->lcd_ctrl.lcd_va_height;
    lcd_ll_fifo_reset(lcd.hal.dev);
    // [run84] auto_next_frame=true 时 batch0 结束瞬间引擎自动开跑 batch1
    // 行周期, 而 vsync ISR 有 1-3 行周期服务延迟 → 早跑的行落在错误
    // 物理行(屏显实证末批整体下移 2-5 行)。改 false: 引擎批间停住,
    // 由边界 ISR 的 start+delay+CKV 硬同步相位。
    ...
    // [run79] 对齐上游 main 启动序：CKV 先行，STV 隔一行线周期拉起，
    // 引擎最后启动；自旋锁内完成保证相位确定（卖家旧序：STV→CKV→
    // 延迟→引擎，在 IDF 5.5 上相位漂移）。
    ...
    g_dbg_va_after_start = lcd.hal.dev->lcd_ctrl.lcd_va_height;
...
static void init_ckv_rmt() {
  ...
  gpio_ll_func_sel(&GPIO, lcd.config.bus.ckv, PIN_FUNC_GPIO);
    ...
    if (intr_status & LCD_LL_EVENT_VSYNC_END) {
        ...
        if (lcd.batches >= batches_needed) {
            ...
            if (lcd.batches == batches_needed - 1) {
                lcd_ll_enable_auto_next_frame(lcd.hal.dev, false);
                // [run83] vfp 10→1: 屏显实证末批物理位置整体下移≈10 行
                // ("1000" 标签与线错位), 与 vfp=10 精确吻合; 中间批一直
                // 是 vfp=1 且位置正确。CKV 公式同步对齐中间批。
                lcd_ll_set_vertical_timing(lcd.hal.dev, 1, 0, vertical_lines % LINE_BATCH, 1);
                g_dbg_va_bnd_rb = lcd.hal.dev->lcd_ctrl.lcd_va_height;
            ...
            // [run79] 回归上游 main 的 ISR 直启（run77/78 的 gdma 重武装
            // 方案废弃：与 eof ISR 竞态致 eof 随机冻死）。
            // [run82 实证回退] 边界 stop+fifo_reset 虽把 eof 修到精确 270/帧,
            // 但破坏 eof/bounce 轮转同步: k>=1 相位仅消费 168 行。废弃。
            lcd_ll_start(lcd.hal.dev);
...
static esp_err_t s3_lcd_configure_gpio()
{
    ...
    for (size_t i = (16 - lcd.config.bus_width); i < 16; i++) {
        gpio_ll_func_sel(&GPIO, DATA_LINES[i], PIN_FUNC_GPIO);
        gpio_set_direction(DATA_LINES[i], GPIO_MODE_OUTPUT);
        esp_rom_gpio_connect_out_signal(DATA_LINES[i],
                                        LCD_PERIPH_SIG(data_sigs[i]), false, false);
    }
    gpio_ll_func_sel(&GPIO, lcd.config.bus.leh, PIN_FUNC_GPIO);
    ...
    gpio_ll_func_sel(&GPIO, lcd.config.bus.clock, PIN_FUNC_GPIO);
    ...
    gpio_ll_func_sel(&GPIO, lcd.config.bus.start_pulse, PIN_FUNC_GPIO);
    gpio_set_direction(lcd.config.bus.start_pulse, GPIO_MODE_OUTPUT);
    esp_rom_gpio_connect_out_signal(lcd.config.bus.start_pulse, LCD_PERIPH_SIG(de_sig), false, false);
    return ESP_OK;
}
...
void IRAM_ATTR epd_lcd_init(const LcdEpdConfig_t* config, int display_width, int display_height) {
    ...
    // [run79] 对齐上游 main：16bit 总线同样每行 2 dummy 字节（卖家旧逻辑
    // (bus_width==8) 在 IDF 5.3+ 的 dma_read_stride 语义下不成立）
    // [run85 实证] dummy=2 时屏显整帧右移 8px(左缘白隙+右边框被裁):
    // IDF 5.5.3 外围本身已在行首插一空字, dummy+额外 LE 造成双插。
    // 单变量回退 dummy=0; stride/reset/启动序保留(边界修复真源)。
    dummy_bytes = 0;
    ...
    ret = esp_intr_alloc_intrstatus(LCD_PERIPH_SIG(irq_id), isr_flags,
    ...
    // [run79] 上游 main：IDF 5.3+ 必须显式设 DMA 读步长
    ...
    // [run81] 回退 false：run80 实证 always_on(true) 下 FIFO 按 pclk 连续
    // 排空（eof=500/帧≈2000 行槽）不与行边界对齐，图像必错位；
    // 边界恢复能力待验证是否依赖它（其余 run79 初始化改造保留）。
...
void epd_lcd_set_pixel_clock_MHz(int frequency) {
    ...
    // set pclk
    // [InkWord 编译适配] IDF >= 5.3 签名变化：第 4 参改为 clk_div 出参，
    // 且分频系数需回写 LCD_CAM 时钟寄存器。
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 3, 0)
