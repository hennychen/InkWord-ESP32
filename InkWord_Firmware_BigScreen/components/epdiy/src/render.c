...
void epd_renderer_init(enum EpdInitOptions options) {
    ...
    for (int i = 0; i < NUM_RENDER_THREADS; i++) {
        ...
        RTOS_ERROR_CHECK(xTaskCreatePinnedToCore(
            ...
            // [run74 修复] 栈 2K→4K：真机实证 epd_prep 栈溢出（feed 路径
            // 含 printf/esp_rom_printf/LUT/preload，2K 裕量不足），溢出
            // 踩坏邻接 heap（DMA 描述符）→ GDMA 停摆 → EOF 冻死随机行。
            configMAX_PRIORITIES - 1, &render_context.feed_tasks[i], i % 2));
            // [InkWord run42] 绑核 i → i%2：NUM_RENDER_THREADS=4 时 i=2/3
            // 为非法核号（S3 仅核 0/1），双核均分保持两两成对。
