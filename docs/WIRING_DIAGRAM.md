...
> 均支持 ext1 深睡唤醒；SET/RST 落 IO 域 41/42，
...
| GPIO41 | SET | 遮蔽/揭晓释义（闪卡自测；待机页：轮换下一条引文） | 预留 SRS「记得」评分 |
| GPIO42 | RST | 回到当前模式第一条 | 预留 SRS「忘了」评分 |
...
> **接线自检**：上电后串口有 `nav switch handler started (UP=1 DOWN=2 LEFT=14 RIGHT=15 CENTER=21 SET=41 RST=42)` 日志；
> 按下各方向应有 `NAV_xx SHORT/LONG` 日志（LOG_D 级）。
