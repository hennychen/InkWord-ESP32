...
> **状态** | 定稿（2026-08-24，v1.4 T4.6 决策不实施）；v1.5 §四已实施（2026-08-25，T5.3 交付：Account/OwnerId/注册登录/卡组 CRUD/App 编辑器/固件零改动红线均兑现）；v2.0 §五已实施首增量（2026-08-25：Device.UserId 兑现 + 绑定端点 + LWS 聚合只读 + 设备 401 自愈，深度漫游留 v2.x）
...
- **绑定**：App 登录后扫码/输 DeviceKey 绑定 → 写 `Device.UserId`（既有可空列兑现）+ 设备 ApiKey 换发（设备重注册语义，MAC 幂等先例沿用）。2026-08-25 实施细化：凭 LAN /api/stats 新增 `mac` 字段定位设备（免手输 DeviceKey，与 sync_register MAC 同源）；绑定后旧钥即刻失效，设备下周期 401 清钥重注册取回新钥，新钥不回传 App（自愈闭环）。
...
- **生态**：卡组商店/UGC 分享/付费挂 Account（依赖 v1.5 归属地基 + v2.0 绑定）。UGC 分享首增量已交付（2026-08-26）：Deck.IsShared 分享开关 + 发现页 + fork 深拷贝导入（独立副本，关闭分享不回收）；商店/付费留待后续。
...
| v2.0 | Device.UserId 兑现 + 绑定端点 + LWS 聚合同步服务（✅ 2026-08-25 首增量：MyDeviceController bind/unbind/aggregate + t20Sql 纯 Id 化迁移） | 设备管理/绑定流程（✅ 2026-08-25：设备页「我的设备」卡，LAN 一键绑定/解绑） | ApiKey 换发消费（✅ 2026-08-25：sync_client 401→SYNC_ERR_AUTH + sync_recover_auth 自愈；stats 端点 +mac） |
