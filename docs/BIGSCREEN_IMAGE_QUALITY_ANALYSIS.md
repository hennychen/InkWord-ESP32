...
| run113a | 白 → 黑 → 图 | 残影清除，但**黑粗条纹**重（纯色黑帧 LCD 排水） |
| **run113b（定稿）** | **白 → 黑 → 白 → 图** | **效果最好：残影清除，仅轻微灰纹（用户终审采用）** |
| run113c | 白 → 反相图 → 图 | 条纹源头消除但净残影弱于 113b，弃用 |

**定稿结论**：黑帧的满摆幅深驱对残影清除贡献最大，轻微灰纹为可接受代价
（规格书 CR 上限 16，面板物理极限）。深清成本 ~17.5s（4 段 GC16），仅在
binfast 模式按周期自动触发（默认每 2 次，`/wf?bfn=N` 可调）；结束后从
s_raw 重解包恢复上传图。
