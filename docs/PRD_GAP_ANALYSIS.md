...
| 中 | 播放读音 | 短按=发音 / 长按=功能菜单（2026-08-23 快捷菜单，配网降为菜单项） | ✅ 兼容 |
| 左 | 自评"忘记" | 短按预留 / 长按=AP 配网门户 | ⚠️ 自评语义未落地（现由 SET 遮蔽/揭晓承载） |
| 右 | 自评"简单" | 短按预留 / 长按=LAN 接收页 | ⚠️ 同上 |
| 长按中 | 收藏 | ✅ 已解决（2026-08-23 菜单化重构）：长按中=功能菜单，收藏列表为菜单项①，配网降为菜单项③；收藏词级动作绑 SET 长按，浏览入口在菜单 | ✅ 已解决 |
| SET 侧键（GPIO42） | **PRD V2.0 未定义**（V2.1 §5.3 已补） | 短按=翻义/轮换引文 / 长按=收藏/取消当前词（收藏视图内取消后移出序列） | ✅ V2.1 已定义并落地 |
| RST 侧键（GPIO40） | **PRD V2.0 未定义**（V2.1 §5.3 已补） | 短按=回第一条 / 长按=临时视图进出（错词本/收藏浏览，2026-08-23 升级） | ✅ V2.1 已定义并落地 |

**结论**：五向 + SET/RST 共 7 键、长短按共 14 个语义槽位，容纳 PRD 5.3 全部映射绰绰有余；
~~PRD 5.3 把"收藏"绑在长按中键上与现行配网功能冲突，且完全遗漏 SET/RST 两个已接线实体键。
需要一次交互语义总表重排~~ **已解决（2026-08-23 快捷菜单落地）**：长按中=功能菜单
（menu_ui 六项：收藏列表/模式选择/配网/AP 门户/LAN/设备信息），收藏绑 SET 长按、
错词本/收藏浏览绑 RST 长按（临时视图进出），交互总表已随菜单重构定稿
（见 [PRD_V2.1.md](PRD_V2.1.md) §5.3/§5.7 与 [MENU_DESIGN.md](MENU_DESIGN.md)）。
