...
|---|---|---|---|---|---|
| MID 416x240 | 186px | 4 列 / ~99px | 56px | 3 | ✅ 良好（全貌 3 组头行+5 格行 ≈ 2 屏滚动） |
| MID 400x300 | 246px | 4 列 / ~95px | 56px | 4 | ✅ 良好 |
| MID 320x240 | 186px | 3 列 / 96px | 56px | 3 | ✅ 良好（3.1" 横屏默认形态，2026-09-19 宽度派生核算；格宽 96 ≥ 5 字标签 80px 阈值） |
| MID 240x320 竖 | ~266px | 2 列 / ~116px | 56px | 4 | ✅ 良好（3.1" 改竖屏方向后形态，待真机校准） |
| SMALL 264x176 | 122px | 2 列 / ~128px | 56px | 2 | ⚠️ 可用但滚动 ~5 屏，文档注记不禁用（用户选择权） |
| TINY 122/128 宽 | — | 2 列仅 ~53px/格，标签被迫缩 2 字 | — | — | ❌ **强制列表**（TINY 缩写不可识别教训，2026-08 先例）；切换键短震反馈不生效 |

- 列数运行期派生（`cols = clamp(avail_w / 90, 2, 4)`，纯宽度派生不参 form 轴——竖屏 avail 自然小、自动降 2 列）；标签字号全档统一 16px（level 0；图标主视觉下 5 字标签 80px 宽是最小格约束，设计稿 MID 横 20px/SMALL 3 列实测放不下，实施修订）
- 格内竖排：图标 20px 居中 + 间距 4 + 标签水平居中；格高定值 56px（`MU_GRID_CELL_H` = pad8 + 图标20 + 间距4 + 标签16 + pad8），组头行高 24px（`MU_GRID_HDR_H`）——行高不均，滚动为**像素级**滑动窗口（`s_goff` 偏移裁剪，非设计稿格行级窗口）
...
- 滚动：像素级滑动窗口跟随光标（行高不均——组头 24/格行 56，`s_goff` 像素偏移 + 窗口裁剪绘制；不画滚动条）；组头行随窗口滚动自然进出
...
| # | 项 | 说明 |
|---|---|---|
| 1 | 图标补齐 | ✅ 19 项全图标：master 印章（方框挖空+中心实心方）+ volume 喇叭（poly+声波弧）；墨封录由 collected 星标换 master 印章（宫格主视觉去重） |
| 2 | 黄金帧 mask | ✅ 实施取「整体动态区」分支：宫格页 = 标题栏下整屏（自检 env NVS 空默认列表不触达，防御性） |
| 3 | 按键说明页 | ✅ 菜单内组三行：「上/下 移动选择·宫格跨行」改写 + 新增「左/右 宫格行内移动」「SET长 切换列表/宫格」 |
| 4 | 文档同步 | ✅ 本节回填 + PRD §5.7 + README 模块表/功能菜单节 + 按键说明页 |

### 12.8 实施任务分解（2026-09-07 全部完成，构建 11 生产 env 零新警告）
...
- MID 横 4 列/SMALL·MID 竖 2 列派生正确；TINY 长按 SET 短震不切换；SMALL 滚动体验真机评估（不佳则档位注记降级讨论）
