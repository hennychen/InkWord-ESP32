...
| | |
|---|---|
| **版本** | v1.2（已实施；v1.1 实施菜单六项，v1.2 增补按键说明页） |
| **日期** | 2026-08-23 |
...
| 设置页（字号/每日新词量/发音/震动开关） | 未实现 | 无处安放；学习页字号无调节手段（阅读模式有） |
| 词书/词库切换 | 未实现 | PRD §7.1 规划 SD 多词库 + 云端下发，设备端无入口 |
| 设备信息（版本/IP/电量/存储） | 无 | 排障靠串口，用户不可见 |
...
```
┌─────────────────────────────┐
│ 功能菜单                1/6 │ ← 标题栏（白底黑字，同学习页状态栏风格）
├─────────────────────────────┤
│ ▶ 收藏列表               12 │ ← 列表区（反选高亮 = 黑底白字）
│   模式选择          闪卡 ●  │
│   Wi-Fi 配网        已连接  │ ← 左 CJK 标签 + 右 ASCII 徽标
│   AP 配网门户               │
│   LAN 接收页                │
│   设备信息                  │
├─────────────────────────────┤
...
```c
typedef struct {
    const char *label;                      /* CJK 标签 */
    void (*badge)(char *buf, size_t n);     /* 可选：右侧徽标填充 */
    void (*activate)(void);                 /* 中键确认动作 */
} mu_item_t;
```

| # | label | badge | activate |
|---|---|---|---|
| 1 | 收藏列表 | `n`（collected_count，0 显示「空」） | 进收藏浏览视图（§6.3） |
| 2 | 模式选择 | 当前模式名 | 进二级列表（§7.2） |
| 3 | Wi-Fi 配网 | 已连接/未连接 | `menu_ui_exit(); wifi_config_ui_enter();` |
| 4 | AP 配网门户 | — | `menu_ui_exit(); lan_portal_enter();` |
| 5 | LAN 接收页 | — | `menu_ui_exit(); lan_server_enter_receive_page();` |
| 6 | 设备信息 | — | 切 INFO 页（§7.3） |

二期预留：设置（字号/每日新词量/发音/震动开关）、词书选择——数组追加即可。
...
静态只读页，CJK 标签 + 混排值，逐行 16px：

```
固件版本   1.0.0
IP 地址    192.168.10.192（未连接则「--」）
词库       2407 词 · 内嵌（SD 优先时标注 SD）
收藏/错词  12 / 3
PSRAM      8.0 MB 可用 3.4 MB
电量       87%（如有 ADC 读数）
运行时长   02:14:36
```

数据源：`FW_VERSION` 经新增 `fw_version()` getter 导出（原为 main.cpp 文件内宏，INFO 页取不到）；IP=`wifi_get_sta_ip()`（已存在，无需新增）、词库/收藏错词/PSRAM/uptime 均有现成 getter；**电量行一期省略**（无 ADC 读数，显示占位值误导排障，二期随电量计补）；词库来源（SD/内嵌）标注二期随词书切换补。

### 7.4 按键说明页（v1.2 增补）

主菜单第 7 项，表驱动分页只读页（22 行 × 按档位派生每页行数：MID 7/SMALL 6/TINY 11），四组内容：学习页（含 `*` = 已收藏标记行）/收藏·错词视图/待机页/菜单内。行文全半角标点（字库安全先例），「短按 / 长按」两段式。上/下翻页循环、中=下一页、SET 返回主菜单、RST 退出菜单；翻页局刷，标题栏右上页码。TINY 档值列宽不足，bring-up 后改单列两行/键（代码已注记）。
