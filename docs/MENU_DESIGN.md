...
主菜单 [系统] 组「设置」项（第 10 项）或任意学习态 **RST 短按直达**（2026-08-27，原「回当前模式首条」退役——低频可由多次上键等价达成；quiz/AI 对话等临时视图 RST 语义不变）进入。覆盖层范式三件套（`settings_ui_enter/is_active/on_button`），镜像 menu_ui：无 init、静态零初始化、无阻塞，`on_button` 转发链在 menu_ui 之后。行项八条（v1.2 四条 + T5.1 测验快答 + T5.5 考试倒计时 + 屏幕方向 + 音量）：
