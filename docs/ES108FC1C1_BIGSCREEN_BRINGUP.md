...
| 灰阶能力 | fb 为 4bpp = **16 级灰**，页面打包顺序与 fb 布局逐位吻合（§14.2）；run100 起 4bpp 上传默认经设备侧 FS 二值化兜底再入 fb（见下行） |
| gray16 直出塌缩 | 上屏几乎全白、仅纯黑核幸存——LCD 路径不执行波形 phase_times（§15）；run100 已上页面默认 1bit FS 抖动 + 设备侧 FS 二值化兜底（`/dither` 可关），正解=扫描量子化专用波形（§15.4） |
...
9. **16 级灰阶的视觉质量未量化**：run99 已证实 4bpp 数据链路端到端打通（§14.5），但灰阶层次是否受 VCOM 固定偏移压缩（同 §10.3 灰染机理）需灰阶标板照片判读；若层次不足，先做 §7.2 VCOM 校准而非改波形。**2026-09-15 已实测闭环：非 VCOM 压缩而是灰阶塌缩（中间灰阶全白），根因是 LCD 路径不执行 phase_times，见 §15；run100 已上 dither 兜底，VCOM 校准顺延为专用波形标定的配套步骤。**
...
## 15. gray16 灰阶塌缩：根因、证据与分级修复（2026-09-15，run100）

> 本节编号说明：本文件 §13/§14 原文缺失（代码注释引用的 §13.x/§14.x 为作者侧版本），为不破坏引用完整性，新结论直接编号 §15。

### 15.1 现象

gray16（4bpp 1036800B）上传：浏览器预览（下载 PNG）灰阶正常，屏显**几乎全白**——仅纯黑小点幸存为深点、深灰线条呈极浅细痕、中灰块完全不可见；几何/朝向/跨批边界全部正确。1bit FS 抖动上传则屏显正常（run99 验收路径）。

### 15.2 根因：LCD 并行路径不执行波形 phase_times

- `prepare_context_for_next_frame`（output_common/render_context.c）把 `phase_times[k]` 存入 `ctx->frame_time`，但 `frame_time` **仅被 I2S 路径消费**（`i2s_write_row`）；`output_lcd/` 全目录零引用 → LCD 路径每相位时长 = 一次 1080 行全扫描 ≈ **110ms**（§4 帧节奏；GC16 30 相位 ≈3.3s = 30×110ms，与意图总时长 1570ms 不符）。
- ED047TC1 GC16 白底跃迁结构（`tools/decode_waveform.py` 离线解码）：前 15 相位驱黑饱和 + 后 15 相位中前 `to` 个相位驱白回修定级，回修意图时长仅 8~20ms/相位（to=8 全部白修意图 70ms）。
- 实际白驱 = `to`×110ms：**to≥5 即超过波形从黑到全白的意图白驱总量（550ms）→ 按全白剂量驱白 → 灰阶塌缩**；to=1~4 得 20%~80% 全白剂量 → 极浅灰；to=0 无白修相位 → 黑幸存；to=15 黑15+白15 对称饱和 → 白。与照片特征逐条吻合。
- 二值内容（全白/全黑/四象限/1bit 抖动）为饱和型跃迁，对相位时长不敏感 → **免疫**，故 run88-run99 全部验收未暴露；卖家原库 epdiy_seller_ref 的 LCD 路径同样忽略 phase_times → 管线族结构性行为，非本 patch 回归。

### 15.3 次要观察（非本病根，留档）

- highlevel.c L134-135 复位 `previously_white/black` 强制走 `MODE_PACKING_1PPB_DIFFERENCE` 分支；与 `PREVIOUSLY_WHITE` 分支色调等价（同一波形码表），仅损失优化路径。
- VCOM 固定偏移（§10.3）为叠加因素，专用波形标定时配套处理（§7.2 电位器）。

### 15.4 分级修复

| 档 | 内容 | 状态 |
|---|---|---|
| 1 | 页面默认 1bit FS 抖动（预览即屏显）；gray16 按钮保留并加塌缩提示 | run100 ✓ |
| 2 | 设备侧 FS 二值化兜底：4bpp 上传先整数 FS 抖动再入 fb（阈值 128 与页面一致）；`/dither` 运行时切换、`/status` 含 dither 字段 | run100 ✓ |
| 3 | 正解：扫描量子化专用波形——最小相位=1 扫描(~110ms)，按 110ms 量子重排 GC16（黑饱和 N 扫 + 白回修 M 扫定级），`tools/gen_gray_ramp.py` 灰阶标板上传→拍照→调 M 映射收敛；随后 §7.2 VCOM 电位器校准。短于扫描的意图相位物理不可实现，长相位可用重复扫描补齐 | 待标定 |

### 15.5 验证规程

1. 塌缩签名复证：`/dither` 关 → 上传 `gray_ramp_gray16.raw` → 预期仅带 0 黑、带 1~4 渐隐、带 ≥5 全白；
2. 兜底生效：`/dither` 开（默认）→ 同标板 16 带以抖动纹理全部可辨；1bit 标板恒正常；
3. 每次烧录后串口确认 `BUILD run100` 与 `recv ok: ... dither=` 签名（§13.4 教训）。
