...
| **音频硬件** | **ES8311+NS4150B ✅ 播放链路验收通过（2026-08-27）**：I2C 控制 38/39、MCLK=GPIO0 实线（必接）、I2S 播放 4/5/6 + 录音 DOUT=11（BS 省线后，待 bring-up）；驱动对齐 esp_codec_dev 1.5.6（REG00 修复档案见 WIRING §2.2）；MP3 帧级采样率动态切换 + 音量 0~100（菜单/设置页/RST 三入口） |
...
**2026-08-26 滚动更新**：2.9" 维峰 WF0290T5PCZ10230H BW 屏 bring-up 收官
（UC8151D 三轮收敛，全刷 3370ms + 局刷 869ms + 保养全刷阈 4，详见
[PANEL_COMPAT_DESIGN §3.2](PANEL_COMPAT_DESIGN.md)）；附带清除两处刷新策略
硬编码（flush_window passes / scheduler 阈值均改读面板 desc）。

**2026-08-27 滚动更新**：① ES8311 播放链路收官（REG00 挂起态根因 +
MCLK 实线定稿，WIRING §2.2）；② 默认词库英文 2134 条真人读音 MP3
整理回填（dictionaryapi.dev/有道混合源，tools/default_vocab/fetch_audio.py，
SD 卡 `/sdcard/audio/` 即用）+ SD 词库 PSRAM 峰值修复（word_parser raw
按需分配，boot 提速 5.6×）；③ I2S 变调双坑修复（ONLY_LEFT 单槽帧 /
i2s_write_mono 截断丢样）；④ 音量功能三入口（菜单 [系统]「音量」页 /
设置页第 8 行 / RST 短按直达设置，es8311 数字音量 0~100 默认 75=0dB）。
详见 [MENU_DESIGN §7.5](MENU_DESIGN.md) 与固件 README §1.3。
...
| 7 | **TINY 档版面微调**（余 3 项）：INFO 值列超宽、拼写线 ~8 位截位、按键说明单列化。2026-08-25 已前置修复 5 项：设置页 h<200 屏高启发式改 LAYOUT 档位判定（+TINY 短标签/提示栏省略）、LAN 接收页与 AP 门户页 TINY 紧凑版式（URL size1 折行）、古诗默写 16px 空框分行（五言单行 5 框）、QUIZ TINY 恒纵列+T3 长按中键重播、菜单 Wi-Fi 项 TINY 重定向 AP 门户；三 env 构建绿（Flash 78.3% 同基线） | 代码内 3 处「bring-up 再调」注记；三屏适配分析结论（e042bw MID 零改动 / **wft0290 已 bring-up 2026-08-26** / opm021eb） |
...
| 2 | **账户体系实施**：UserId 挂接 + 多设备同步（决策已于 v1.4 #6 定，多设备同步 LWS 策略已有）。2026-08-25 首增量交付（软件侧全栈，真机验收待硬件）：后端 MyDeviceController（api/me/devices list/bind/unbind + progress/aggregate LWS 聚合只读；bind 凭 LAN stats mac、幂等不重复换发、绑他人 409）+ Device.UserId 纯 Id 化（删 User 旧导航防惯例 FK 误指 Users；t20Sql 幂等迁移拆存量 FK + UserId 索引）+ DeviceAccountTests 5 用例（65/65 绿）；固件 stats 端点 +mac（绑定凭据）、sync_client 401→SYNC_ERR_AUTH、main sync_recover_auth 清钥重注册自愈（上报 flush/心跳/静默心跳三链路接入）；App CloudDevice/CloudAggregate API + 设备页「我的设备」卡（LAN 一键绑定/解绑）；深度进度漫游（设备拉取聚合影子）留 v2.x | PRD_GAP §四；ADR-001 §五/§六 |
| 3 | **卡组生态**：教材包商店 / UGC 分享 / 付费卡组（依赖账户与 v1.5 G4 闭环）。2026-08-26 UGC 分享首增量交付（软件侧，真机验收待硬件）：后端 Deck.IsShared/SharedAt + t21Sql 幂等迁移（部分索引）+ share 开关 / decks/shared 发现页（SharedAt 倒序 + 名称/科目过滤 + 分页 +1 探测）/ decks/{id}/fork 深拷贝导入（CopyWord 纯函数：内容全量拷 + 归属重定向 Tag/SubjectId/DeckId + Ai 审校状态不渗入；官方卡组可 fork 改造，他人未分享 404 防存在性探测；导入是独立副本，关闭分享不回收）+ DeckShareTests 4 用例（69/69 绿）；App CloudDeck +isShared/ownerName + sharedDecks/setDeckShared/forkDeck（forkDeck 走 _send 直发避 _post cast 陷阱）+ 编辑器我的卡组分享开关（确认对话框明示公开范围）+ 「发现卡组」页（搜索/下拉刷新/分页/一键导入 + SnackBar 查看）；设备零改动——fork 后走既有 LAN 推送/增量同步通道；商店/付费留待后续（支付合规未启动） | G6 |
...
- **AI 学习支线（A1-A3 交付 2026-08-29，三云 E2E + 双语 ASR 同日验收）**：场景化对话 + 英→中翻译（[AI_CHAT_MODE.md](AI_CHAT_MODE.md) §7 协议向后兼容，query 可选）+ 中文 TTS 双音色路由（**双语 ASR 已部署达标：zipformer-zh-en 20 句 CER 1.3%，中→英闭环实测通过，§7.3**）+ ChatTurn 落库/对话周报（周日 05:00 Job + chat-review 端点 + 设备「对话周报」菜单页）+ 对话生词命中 SET 收藏（复用 /sync/collect 零新协议）；后端 113/113 绿、固件 8 env 绿；**云 LLM 切换位验收：DeepSeek 主力/Qwen 备选/智谱兑底（§8 实测，含 emoji 剧显缺陷修复）**；实机验收：ES8311 模块已集成（取代 INMP441），待 bring-up（v1.1 #1）；A4 思路池（听力小课/例句跟读纠错/AI 测验增强/作文批改/家长周报/剧本 UGC）按价值排期。
...
> 各版本「真机验收待硬件」项已归集至 [ACCEPTANCE_MATRIX.md](ACCEPTANCE_MATRIX.md)（按链路分组 + 拓扑序，硬件到手即执行）；新版本待验收项登记至该矩阵，不再仅在交付行内备注。

| 版本 | 状态 | 计划 vs 实际差异 |
|:---:|:---:|:---|
| v1.0（本文档基线） | ✅ 2026-08-24 | — |
| v1.1 真机闭环版 | 🟡 软件项完成 2026-08-24，真机项待硬件 | T1.5 NVS 扩容（0x30000 + LR_SPARSE_MAX 4000）/ T1.6 提示音（ui_sfx + 4 样本 + 埋点）/ T1.8 开机埋点已交付且构建绿（RAM 18.7%/Flash 77.4%）；T1.1 ES8311 bring-up / T1.2-T1.4 真机回归与功耗实测 / T1.7 TINY 微调 2026-08-25 前置修复 5 项（余 INFO 值列/拼写线截位/按键说明单列化待硬件窗口）（不阻塞后续版本软件开发） |
...
| v1.6 配套（三端功能补齐 P1+P2+P3） | ✅ 软件侧完成 2026-09-08（后端 148 测试全绿 / Angular ng build 绿 / Flutter analyze 0 issue + 22 测试绿），端到端联调待硬件 | 缺口分析：后端设备/管理 API 已 100% 覆盖固件 v1.6 协议（BACKEND_PROTOCOL.md 17 端点全齐），缺口集中在管理端 UI / App 报告视角 / 卡组运营三处，据此补齐：**P1 管理端阅读器配套**（纯 Angular 后端零改动）——书籍管理模块 /books（book-api 对接 AdminBook 6 端点 + 上传对话框 TXT/MD/HTML ≤10MB + 发布开关/删除确认/详情阅读统计）+ 看板「阅读分析」区（readingStats 统计条今日活跃读者/阅读时长/已发布书数 + 热门书籍 Top10 + 近 7 天双 Y 轴趋势 ECharts）+ 设备详情「阅读记录」卡（进度条/当前页/累计分钟）；**P2 App 学习报告闭环**——后端 MyDeviceController 扩展 me 三端点（chat-review 归属校验 404 防探测 + limit 钳制 + PayloadJson 五段解析非法 JSON try-catch 兜底 / reading join Books 按 LastReadAt 倒序 / books 同 GetPublishedAsync 单源口径）+ progress/aggregate 响应增补 masteredCount（跨设备 WordId Distinct 去重，同词多设备只计一次）+ DeviceController.ToPayload 非法 JSON 抛 JsonException 同源隐患顺手修复；App stats_page 升级「学习报告」（LAN 本地统计视图保留 + 云端分区登录态可见：设备选择器/周报卡五段解析+reviewWords chips+历史周切换 limit 分页/聚合卡墨封数/阅读卡进度条/云端书库只读列表引导设备端下载），home_shell「统计」→「报告」；**P3 管理端卡组/科目运营**——后端 AdminSubjectController 新建（subjects 列表含每科目卡组数）+ AdminDeckController 扩展（List 增科目过滤/Owner/共享态，Detail 学习覆盖统计 learnerCount/studiedItems，Items 条目分页含归档行，条目 CRUD 复用 MyDeckController.FillWord/NextVersion 公共方法同源禁复制——Version 接全局 max 递增设备增量同步契约保持，uq(Text,Tag) 冲突 409，Tag=deck.Code 隔离）；管理端 /decks 卡组管理页（科目 chip 侧栏过滤含计数 + Deck 表 + 条目管理弹窗大宽分页/行内编辑版式动态标签 qa/poem 字段同名映射 + 字符集核对对话框拉 T4.5 charset 端点展示字符流可复制/下载喂 gen_cjk_font.swift + AI 生成卡组入口自词库页迁入）；测试：AccountTests 增 NextVersion Theory 4 例（max 语义/防回退/冷启动）、DeviceAccountTests 增 ReviewPayload 三态/ClampReviewLimit Theory 7 例；留集成验证（项目纯逻辑直测范式，无 EF/认证管线基建）：归属 404/空周报 404/阅读列表映射/me books Published 过滤/条目 CRUD uq 409 与 Roles 白名单/aggregate 零设备分支；真机验收（管理端上书→设备下载阅读→进度上报→看板/App 报告可见全链 + 周报 ChatReview 链 + 管理端条目 CRUD→设备增量同步链）待硬件 |
| v2.0 平台化版 | 🟡 账户实施首增量交付 2026-08-25（§七 #2 软件侧全栈），真机验收待硬件 | #2 账户体系实施首增量（ADR-001 §五兑现）：后端——MyDeviceController 新建（api/me/devices list/bind/unbind + progress/aggregate 只读聚合；bind 凭 LAN stats mac 定位设备、幂等——已绑本账户直接成功不重复换发防 App 重试风暴踢设备下线、绑他人 409；unbind 不换发 ApiKey 设备零感知回无主态）+ LWS 聚合 LastStudiedAt 新者**整行**胜出（FSRS 状态不可拆分合并；深度进度漫游留 v2.x）+ Device.UserId 纯 Id 化（删 User 导航防 EF 惯例 FK 误指 Users 管理表，Word→Subject/Deck T4.1 同先例；t20Sql 幂等迁移：ADD COLUMN + DO $$ 动态拆存量 Devices→Users FK + UserId 索引）+ DeviceAccountTests 5 用例（bind 幂等/他人 409/LwsMerge 乱序与截断）；固件——stats 端点 +mac 字段（ESP_MAC_WIFI_STA 大写 12hex 与 sync_register 同源，绑定凭据）+ sync_client 四函数（pull/push/collect/heartbeat）401 分支返回 SYNC_ERR_AUTH(-2) 与通用失败 -1 区分 + main.cpp sync_recover_auth 自愈（清内存/NVS 钥→MAC 幂等重注册取回新钥；上报两处 + 心跳两处（静默会话/后台任务）接入）；App——CloudDevice/CloudAggregate + 四设备 API（bindDevice 走 _send 直发规避 _post cast<String,String> int/bool 陷阱）+ 设备页「我的设备」卡（登录态 LAN 一键绑定/清单/解绑；新钥不回传 App——401 自愈闭环使写回通道多余）；构建态：后端 sln 0 错 + 65 测试全绿、inkword-s3 绿（Flash 78.3% +496B）/wft0290 绿、native 31 绿 + 3 skipped、Flutter analyze 0 issue + 22 测试绿；余项处置：#1 ESP-IDF 迁移评估 / #4 摇杆评估 / #5 结构量产为硬件依赖评估项（无硬件不动）；#3 卡组生态依赖的账户地基本轮已就绪，商店/付费留待后续；2026-08-26 #3 UGC 分享首增量已交付（share 开关/发现页/fork 深拷贝，后端 69/69 绿 + App analyze 0 issue/22 测试绿，设备零改动） |
