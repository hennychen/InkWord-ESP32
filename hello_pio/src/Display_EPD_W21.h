...
void EPD_Full_PartUpdate(const unsigned char* datas);  //全屏局部刷新
void EPD_Window_PartUpdate(unsigned int px_start, unsigned int py_start, const unsigned char* datas, unsigned int px_width, unsigned int py_height, const uint8_t* lut); //窗口局刷, lut为LUT数组
extern uint8_t PARTIAL_2IN13[];     // 常规LUT(TP[0A]=1帧)
extern uint8_t PARTIAL_2IN13_LAST[]; // 最后一笔LUT(TP[0A]=3帧)
//Num display
